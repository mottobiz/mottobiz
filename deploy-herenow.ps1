# Deploy/Update here.now site (reads API key from ~/.herenow/credentials)
param(
...
if (-not (Test-Path $credentialsFile)) {
    throw "API key not found. Please save your API key to $credentialsFile"
}

$ApiKey = Get-Content $credentialsFile -Raw
$ApiKey = $ApiKey.Trim()
...
# Step 1: Update publish with API key
...
Invoke-RestMethod -Uri $finalizeUrl -Method POST -ContentType "application/json" -Headers @{
    "Authorization" = "Bearer $ApiKey"
    "X-HereNow-Client" = "verdent/direct-api"
...
Write-Host "This is a permanent publish (authenticated)" -ForegroundColor Green
